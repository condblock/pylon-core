...
interface CargoRebarBlock : LogisticRebarBlock, EntityHolderRebarBlock {
    ...
    fun updateDirectlyConnectedFaces() {
        for (face in IMMEDIATE_FACES) {
            ...
            if (otherBlock !is CargoRebarBlock
    ...
    companion object : Listener {
        ...
        private val cargoBlocks = IdentityHashMap<CargoRebarBlock, CargoBlockData>()
        private val cargoTickers = IdentityHashMap<CargoRebarBlock, Job>()

        private fun startTicker(block: CargoRebarBlock) {
        ...
        private fun onBreak(event: RebarBlockBreakEvent) {
            ...
            for (face in block.cargoBlockData.groups.toMap().keys) {
                ...
                if (otherBlock is CargoRebarBlock && face.oppositeFace in otherBlock.cargoBlockData.groups) {
        ...
        private fun onPlace(event: RebarBlockPlaceEvent) {
            ...
            for (face in block.cargoBlockData.groups.toMap().keys) {
                BlockStorage.getAs<CargoRebarBlock>(block.block.getRelative(face))?.updateDirectlyConnectedFaces()
            }
